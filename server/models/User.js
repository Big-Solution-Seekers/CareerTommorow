...
class User {
  ...
  static async list() {
    const query = `SELECT id, username, email, profile_image FROM users`;
    const result = await knex.raw(query);
    return result.rows.map((rawUserData) => new User(rawUserData));
  }

  static async find(id) {
    const query = `SELECT id, username, email, profile_image FROM users WHERE id = ?`;
    const result = await knex.raw(query, [id]);
    const rawUserData = result.rows[0];
    return rawUserData ? new User(rawUserData) : null;
  }
  ...
  static async findByUsername(username) {
    const query = `SELECT id, username, email, profile_image FROM users WHERE username = ?`;
    const result = await knex.raw(query, [username]);
    const rawUserData = result.rows[0];
    return rawUserData ? new User(rawUserData) : null;
  }
  ...
  static async create(username, password, email, profileImage) {
    ...
    const query = `INSERT INTO users (username, password_hash, email, profile_image)
      VALUES (?, ?, ?, ?) RETURNING id, username, email, profile_image`;
